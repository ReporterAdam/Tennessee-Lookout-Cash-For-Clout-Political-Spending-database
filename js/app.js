/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Loads module scripts on demand
───────────────────────────────────────────────────────────────────────────── */
...
(function () {
  ...
  // ── State ───────────────────────────────────────────────────────────────────
  const state = {
    view:     'landing',   // landing | lobbying | campaign | ie | top-spenders
    subview:  null,        // politicians | donors | top | search
    era:      '2012',      // 2012 | 2017 | 2022 | 2025
    entity:   null,        // selected entity key
  ...
  async function render() {
    ...
    // Always render the shell (header + nav) then delegate to modules
    app.innerHTML = renderShell();

    // Attach nav button listeners
    app.querySelectorAll('.tn-nav-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        navigate({ view: btn.dataset.view, subview: null, entity: null, query: '' });
      });
    });
  ...
  function renderShell() {
    ...
    const navHTML = navItems.map(n => `
      <button class="tn-nav-btn ${state.view === n.view ? 'active' : ''}" data-view="${n.view}">
        ${n.label}
      </button>
    `).join('');
  ...
  window.TNApp = { loadData, fmt, fmtFull, eraCol, partyBadge, navigate, renderLoading, renderEmpty, state, BASE_URL, DATA_URL };
