      ...
      <div class="tn-era-filters" style="flex-wrap:wrap;gap:6px;">
        ...
        ${window.TN_IE_ERA_OPTS.map(([y, l]) => `
          <button class="tn-era-btn ${currentEra === y ? 'active' : ''}" data-era="${y}"
            style="${y.startsWith('cycle_') && currentEra !== y ? 'background:var(--tn-yellow-light);border-color:#f0d060;color:#7a5c00;' : y.startsWith('cycle_') && currentEra === y ? 'background:#c8a000;border-color:#c8a000;color:#fff;' : ''}">
            ${l}
          </button>
        `).join('')}
      </div>
