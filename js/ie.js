/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Independent expenditures module:
     - Politicians tab: who money was spent on
     - Spenders tab: who spent the money + who funded them
───────────────────────────────────────────────────────────────────────────── */
...
window.TNIE = (function () {
  ...
  async function render(container, state, helpers) {
    ...
    const subview = state.subview || 'ie-politicians';

    container.innerHTML = `
      <div class="tn-subnav">
        <button class="tn-subnav-btn ${subview === 'ie-politicians' ? 'active' : ''}" data-sub="ie-politicians">
          Politicians
        </button>
        <button class="tn-subnav-btn ${subview === 'ie-spenders' ? 'active' : ''}" data-sub="ie-spenders">
          Spenders
        </button>
      </div>
  ...
  async function renderPoliticians(container, state, helpers) {
    ...
    let rows = pols.filter(r => {
      const matchFilter = filter === 'all' ? true
                        : filter === 'current' ? r.current_elected === 'Yes'
                        : filter === '2026'    ? r.running_in_2026 === 'Yes'
                        : true;
        ...
        <div class="tn-profile-body">
          ...
          <div id="ie-spender-tab-content">
            ...
            ${tab === 'donors' ? `
              ${donors.length ? `
                <div class="tn-methodology">
                  Donors to this group pulled from Tennessee Registry of Election Finance
                  contribution records. Some groups may have additional funding sources
                  not captured in state filings.
                </div>
                <div class="tn-table-wrap">
                  <table class="tn-table">
                    <thead><tr>
                    ...
                    <tbody>
                      ${donors.map((d, i) => `
                        <tr>
                          ...
                          <td class="money">${parseInt(d.num_donations).toLocaleString()}</td>
                        </tr>
                      `).join('')}
                    </tbody>
                  </table>
