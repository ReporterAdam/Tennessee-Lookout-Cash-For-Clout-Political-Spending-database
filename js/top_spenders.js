/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Top political spenders view — cross-dataset lobbying + CF + IE rankings
───────────────────────────────────────────────────────────────────────────── */
...
window.TNTopSpenders = (function () {
  ...
  function eraLabel(era) {
    return { '2012': 'Since 2012', '2017': 'Since 2017', '2022': 'Since 2022', '2025': 'Since 2025' }[era] || 'Since 2012';
  }
  ...
  async function render(container, state, helpers) {
    ...
    container.innerHTML = `
      <div class="tn-methodology">
        <strong>About this ranking:</strong> Combines lobbying spending reported to the
        Tennessee Bureau of Ethics, campaign contributions to Tennessee politicians and PACs,
      ...
      <div class="tn-era-filters">
        ...
        ${[['2012','All Years (Since 2012)'],['2017','Since 2017'],['2022','Since 2022'],['2025','Since 2025']].map(([y, label]) => `
          <button class="tn-era-btn ${era === y ? 'active' : ''}" data-era="${y}">${label}</button>
        `).join('')}
      </div>
      ...
      <div class="tn-result-count">
        ...
        spending $500K+ ${eraLabel(era).toLowerCase()}
      </div>
    ...
    searchInput.addEventListener('input', () => {
      ...
      searchTimer = setTimeout(() => navigate({ query: searchInput.value, page: 0 }), 280);
    });
    ...
    if (moreBtn) {
      moreBtn.addEventListener('click', () => navigate({ page: (state.page || 0) + 1 }));
    }
  }
  ...
  async function renderProfile(container, state, helpers) {
    ...
    if (!row) {
      container.innerHTML = `<div class="tn-empty">Entity not found.</div>`;
      return;
    }

    const hasLob = row.has_lobbying;
    const hasCF  = row.has_cf;
    const hasIE  = row.has_ie;
      ...
      <div class="tn-profile">
        <div class="tn-profile-header">
          ...
          <div class="tn-profile-meta">
            ${hasLob ? '<span>Lobbying</span>' : ''}
            ${hasCF  ? '<span>Campaign Finance</span>' : ''}
            ${hasIE  ? '<span>Independent Expenditures</span>' : ''}
        ...
        <div class="tn-profile-body">
          ...
          <div class="tn-profile-stats">
            <div class="tn-stat-box">
              <div class="tn-stat-label">Total Since 2012</div>
            ...
            <div class="tn-stat-box">
              ...
              <div class="tn-stat-value">${fmtFull(row.grand_total_since_2025)}</div>
            </div>
          </div>
          ...
          <div style="display:flex;gap:12px;flex-wrap:wrap;">
            ${hasLob ? `<button class="tn-subnav-btn" id="ts-goto-lob">
              View lobbying details →
            </button>` : ''}
            ${hasCF ? `<button class="tn-subnav-btn" id="ts-goto-cf">
              View campaign finance →
            </button>` : ''}
            ${hasIE ? `<button class="tn-subnav-btn" id="ts-goto-ie">
              View independent expenditures →
            </button>` : ''}
          </div>
        </div>
      </div>
    ...
    if (ieBtn) {
      ieBtn.addEventListener('click', () => {
        navigate({ view: 'ie', entity: row.entity_name, subview: 'spenders' });
      });
    }
  }
