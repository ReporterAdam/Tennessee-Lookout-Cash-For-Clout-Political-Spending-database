/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Campaign finance module
   Changes: default era 2022, no 2026 races filter, PAC breakdown on profiles
───────────────────────────────────────────────────────────────────────────── */
...
window.TNCampaign = (function () {
  ...
  async function renderPoliticians(container, state, helpers) {
    ...
    const totalCol = `total_total_since_${era}`;
    ...
    container.innerHTML = `
          ...
          <tbody>
            ${visible.map((r, i) => `
              <tr>
                ...
                <td class="money">${fmt(r[`campaign_total_since_${era}`])}</td>
                <td class="money">${fmt(r[`pac_total_since_${era}`])}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      ...
      ${hasMore ? `<button class="tn-show-more" id="cf-pol-more">
    ...
    container.querySelectorAll('.name-link[data-key]').forEach(cell => {
      cell.addEventListener('click', () => {
        navigate({ entity: decodeURIComponent(cell.dataset.key), subview: 'politician' });
      });
    });
  ...
  async function renderPoliticianProfile(container, state, helpers) {
    ...
    const [pols, totals, topDonors, smallDonors, allPols] = await Promise.all([
      loadData('cf_politicians.csv'),
      loadData('cf_politician_totals.csv'),
    ...
    // Find all accounts for this politician (campaign account + PAC)
    const allAccounts = pols.filter(r => r.politician_key === key);
    const meta        = allAccounts[0];
    ...
    // Find PAC account if exists (same fixed_name but account_type = PAC)
    // Look for politician PAC linked to this politician
    const pacEntry = allPols.find(r =>
      r.politician_key !== key &&
      r.display_name &&
      r.display_name.toLowerCase().includes((meta.display_name || '').split('(')[0].trim().toLowerCase()) &&
      r.account_type === 'PAC'
    );
    const pacTotals = pacEntry ? totals.find(r => r.politician_key === pacEntry.politician_key) : null;

    const donors   = topDonors.filter(r => r.politician_key === key)
                              .sort((a, b) => (parseFloat(b[`total_since_${era}`]) || 0) - (parseFloat(a[`total_since_${era}`]) || 0));
    const smallRow = smallDonors.find(r => r.politician_key === key);

    // Totals from the totals file
    const totalRaised   = parseFloat(tot ? tot[`total_total_since_${era}`]    : 0);
    const campaignTotal = parseFloat(tot ? tot[`campaign_total_since_${era}`] : 0);
    const pacTotal      = parseFloat(tot ? tot[`pac_total_since_${era}`]      : 0);
    ...
    container.innerHTML = `
      ...
      <div class="tn-profile">
        ...
        <div class="tn-profile-body">
          ...
          <!-- Campaign account vs PAC breakdown bar -->
          ${totalRaised > 0 ? `
            <div style="margin-bottom:20px;">
              <div style="font-size:12px;color:var(--tn-text-muted);margin-bottom:6px;font-family:var(--tn-font-mono);text-transform:uppercase;letter-spacing:0.06em;">
                Fundraising breakdown
              </div>
              <div style="display:flex;height:12px;border-radius:6px;overflow:hidden;background:var(--tn-border);">
                <div style="width:${(campaignTotal/totalRaised*100).toFixed(1)}%;background:var(--tn-blue);transition:width 0.3s;"></div>
                <div style="width:${(pacTotal/totalRaised*100).toFixed(1)}%;background:var(--tn-accent);transition:width 0.3s;"></div>
              </div>
            ...
            <table class="tn-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td class="money">${fmtFull(tot ? tot.campaign_total_since_2012 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.campaign_total_since_2017 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.campaign_total_since_2022 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.campaign_total_since_2025 : 0)}</td>
                </tr>
                <tr>
                  <td>PAC</td>
                  <td class="money">${fmtFull(tot ? tot.pac_total_since_2012 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.pac_total_since_2017 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.pac_total_since_2022 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.pac_total_since_2025 : 0)}</td>
                </tr>
                <tr style="font-weight:600;border-top:2px solid var(--tn-border);">
                  <td>Total</td>
                  <td class="money" style="color:var(--tn-accent);">${fmtFull(tot ? tot.total_total_since_2012 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.total_total_since_2017 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.total_total_since_2022 : 0)}</td>
                  <td class="money">${fmtFull(tot ? tot.total_total_since_2025 : 0)}</td>
                </tr>
              </tbody>
            </table>
          </div>
                ...
                <tbody>
                  ${donors.slice(0, 25).map((d, i) => `
                    <tr>
                      ...
                      <td class="money">${fmtFull(d[`total_since_${era}`])}</td>
                    </tr>
                  `).join('')}
                </tbody>
              </table>
            </div>
            ${donors.length > 25 ? `<p style="font-size:13px;color:var(--tn-text-muted);">Showing top 25 of ${donors.length} donors</p>` : ''}
          ...
          ${smallRow ? `
            <div class="tn-methodology">
              ...
              <strong>${fmtFull(smallRow[`total_since_${era}`])}</strong>
    ...
    container.querySelector('#cf-pol-back').addEventListener('click', () => {
      navigate({ entity: null, subview: 'politicians' });
    });
    container.querySelectorAll('.tn-era-btn').forEach(btn => {
    ...
    container.querySelectorAll('.name-link[data-donor]').forEach(cell => {
      cell.addEventListener('click', () => {
        navigate({ entity: decodeURIComponent(cell.dataset.donor), subview: 'donors' });
      });
    });
  }
  ...
  async function renderDonors(container, state, helpers) {
    ...
    container.innerHTML = `
          ...
          <tbody>
            ${visible.map((r, i) => `
              <tr>
                ...
                <td class="money" style="font-weight:600;">${fmt(r.total_since_2012)}</td>
      ...
      ${hasMore ? `<button class="tn-show-more" id="cf-donor-more">
        Show more (${rows.length - visible.length} remaining)
      </button>` : ''}
    `;
    ...
    container.querySelectorAll('.name-link[data-key]').forEach(cell => {
      cell.addEventListener('click', () => {
        navigate({ entity: decodeURIComponent(cell.dataset.key), subview: 'donors' });
      });
    });
  ...
  async function renderDonorProfile(container, state, helpers) {
          ...
          ${donor ? `
            <div class="tn-profile-stats">
              <div class="tn-stat-box">
                ...
                <div class="tn-stat-value accent">${fmtFull(donor.total_since_2012)}</div>
              </div>
              <div class="tn-stat-box">
                <div class="tn-stat-label">Since 2017</div>
                <div class="tn-stat-value">${fmtFull(donor.total_since_2017)}</div>
              </div>
              <div class="tn-stat-box">
                <div class="tn-stat-label">Since 2022</div>
                <div class="tn-stat-value">${fmtFull(donor.total_since_2022)}</div>
              </div>
              <div class="tn-stat-box">
                <div class="tn-stat-label">Since 2025</div>
                <div class="tn-stat-value">${fmtFull(donor.total_since_2025)}</div>
              </div>
            </div>
          ` : ''}
                ...
                <tbody>
                  ${recipients.map((r, i) => `
                    <tr>
                      ...
                      <td class="money">${fmtFull(r[`total_since_${era}`])}</td>
                    </tr>
                  `).join('')}
                </tbody>
              </table>
    ...
    container.querySelector('#cf-donor-back').addEventListener('click', () => {
      navigate({ entity: null, subview: 'donors' });
    });
    container.querySelectorAll('.tn-era-btn').forEach(btn => {
      btn.addEventListener('click', () => navigate({ era: btn.dataset.era }));
    });
    container.querySelectorAll('.name-link[data-pol]').forEach(cell => {
      cell.addEventListener('click', () => {
        navigate({ entity: decodeURIComponent(cell.dataset.pol), subview: 'politician' });
      });
    });
  }
