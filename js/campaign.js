/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Campaign finance module:
     - Politicians tab: ranked table + profile drill-down
     - Donors tab: ranked org donors + where money went
───────────────────────────────────────────────────────────────────────────── */
...
window.TNCampaign = (function () {
  ...
  function eraCol(era, prefix) {
    const suffix = { '2012': '2012', '2017': '2017', '2022': '2022', '2025': '2025' }[era] || '2012';
    return prefix ? `${prefix}_total_since_${suffix}` : `total_total_since_${suffix}`;
  }
  ...
  async function render(container, state, helpers) {
    ...
    container.innerHTML = `
      <div class="tn-subnav">
        <button class="tn-subnav-btn ${subview === 'politicians' ? 'active' : ''}" data-sub="politicians">
          Politicians
        </button>
        <button class="tn-subnav-btn ${subview === 'donors' ? 'active' : ''}" data-sub="donors">
  ...
  async function renderPoliticians(container, state, helpers) {
    ...
    // Merge metadata + totals
    const totalsMap = {};
    totals.forEach(r => { totalsMap[r.politician_key] = r; });

    let rows = pols.map(p => ({
      ...p,
      ...(totalsMap[p.politician_key] || {}),
    }));

    // Filters
    const era      = state.era || '2012';
    ...
    if (filter === '2026')      rows = rows.filter(r => r.running_in_2026 === 'Yes');
    ...
    container.innerHTML = `
      <div class="tn-era-filters" style="margin-bottom:12px;">
        ...
        ${[['2012','All Years'],['2017','2017'],['2022','2022'],['2025','2025']].map(([y,l]) => `
      ...
      <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;">
        ...
        <button class="tn-subnav-btn ${filter === '2026'    ? 'active' : ''}" data-filter="2026">2026 Races</button>
      </div>
      ...
      <div class="tn-profile">
        <div class="tn-profile-header">
          ...
          <div class="tn-profile-meta">
            ...
            ${meta.running_in_2026 === 'Yes' ? '<span>🗳️ Running in 2026</span>' : ''}
          </div>
        </div>
        <div class="tn-profile-body">
          ...
          <div class="tn-profile-stats">
            <div class="tn-stat-box">
              ...
              <div class="tn-stat-value accent">${fmtFull(tot ? tot[`total_total_since_${era}`] : 0)}</div>
            </div>
            <div class="tn-stat-box">
              <div class="tn-stat-label">Campaign Account</div>
              <div class="tn-stat-value">${fmtFull(tot ? tot[`campaign_total_since_${era}`] : 0)}</div>
            </div>
            <div class="tn-stat-box">
              <div class="tn-stat-label">PAC</div>
              <div class="tn-stat-value">${fmtFull(tot ? tot[`pac_total_since_${era}`] : 0)}</div>
            </div>
          </div>
  ...
  async function renderDonors(container, state, helpers) {
    ...
    let rows = donors.filter(r => {
      ...
      const hasAmount   = parseFloat(r[col]) > 0;
      return matchSearch && hasAmount;
      ...
      <div class="tn-profile">
        <div class="tn-profile-header">
          ...
          ${donor && donor.politician_reference ? `
            <div class="tn-profile-meta">
              <span>PAC linked to: ${donor.politician_reference}</span>
            </div>
          ` : ''}
        </div>
        <div class="tn-profile-body">
          ...
          ${donor ? `
            <div class="tn-profile-stats">
              <div class="tn-stat-box">
                <div class="tn-stat-label">Total Given (Since 2012)</div>
